...
/**
 * Provides a resource to remove user favorite
 *
 * @RestResource(
 *   id = "remove_user_favorite_rest_resource",
 *   label = @Translation("Remove user favorite rest resource"),
 *   uri_paths = {
 *     "canonical" = "/api/removeuserfavorite"
 *   }
 * )
 */
class RemoveUserFavoriteRestResource extends ResourceBase {
  ...
  public function get() {
    ...
    if ($flagging) {
      $flag_service->unflag($flag, $node, $this->currentUser);
    }
    $responseData['response']['success'] = 'Favorite removed from user';
